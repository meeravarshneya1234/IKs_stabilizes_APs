function deriv = dydt_Ohara_inject(t,statevar,Id,p,c,Inject)
...
%--------------------------------------------------------------------------
                            %% -- dydt_Ohara_inject.m --%%
% Description: Runs the "inject current simulation" for the O'Hara Model
...
if V >= -60 
    dv=-(1/p.Cm)*(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab+Id-Inject);
else
