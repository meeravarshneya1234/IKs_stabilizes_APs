...
flags.ICaL = 1;
% flags.IKs = 1;
IKsBP = [0]; % block IKs Phosphorylation in third iteration 
flags.PLB = 1;
flags.TnI = 1;
flags.INa = 1;
flags.INaK = 1;
...
sims = {'IKsBP'};
ICa_scales = { [10:0.1:15]}; %calcium perturbation for each model based on order of modelnames
start = 10;
...
for index = 1:length(sims)    
    ...
    for ii = 1:length(scale) % for each calcium factor 
        ...
        for i = 1:variations % for each model variant 
            scaling = scalings(i,:);
